...
export default function Login() {
  ...
  const handleVerifyUnconfirmedOtp = async (e) => {
    ...
    try {
      ...
      if (verifyErr) {
        const { error: retryErr } = await supabase.auth.verifyOtp({
          email: otpEmail.trim(),
          token: cleanToken,
          type: 'email'
        });
  ...
  const handleForgotVerifySubmit = async (e) => {
    ...
    try {
      ...
      setOtpError(err.message || 'Failed to verify code or reset password.');
    } finally {
      setOtpLoading(false);
    }
  };
  ...
  const handleResendOtp = async (type = 'signup') => {
    ...
    try {
      if (type === 'recovery') {
        const { error: resErr } = await supabase.auth.resetPasswordForEmail(targetEmail.trim());
      ...
      setOtpSuccess(`A new code has been sent! (${updatedStatus.remaining} attempts remaining today)`);
    } catch (err) {
