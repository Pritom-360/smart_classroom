            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-850 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Sponsor Name</label>
                  <input
                    type="text"
                    placeholder="e.g. Google Cloud"
                    value={newSponsorName}
                    onChange={(e) => setNewSponsorName(e.target.value)}
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:text-white transition-all"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Sponsor Logo (File or URL)</label>
                  {sponsorLogoFile ? (
                    <div className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs h-[38px] transition-all">
                      <span className="truncate font-semibold text-slate-750 dark:text-slate-350 max-w-[150px]">{sponsorLogoFile.name}</span>
                      <button
                        type="button"
                        onClick={() => setSponsorLogoFile(null)}
                        className="text-red-500 hover:text-red-750 font-bold ml-2 shrink-0 cursor-pointer"
                      >
                        Clear
                      </button>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:text-white transition-all"
                      />
                      <label className="bg-white hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all shrink-0 flex items-center gap-1.5 active:scale-95 shadow-sm">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">Website Link (Optional)</label>
                  <input
                    type="url"
              ...
              <div className="flex justify-end pt-1">
                <button
                  ...
                  disabled={uploadingLogo || !newSponsorName.trim()}
                  className="bg-indigo-600 hover:bg-indigo-750 disabled:opacity-50 text-white font-extrabold px-5 py-2.5 rounded-xl text-xs shadow-md shadow-indigo-500/10 hover:shadow-indigo-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer disabled:pointer-events-none"
                >
                  {uploadingLogo ? (
                    <>
                    ...
                    <>
                      ...
                      <span>Add Sponsor to Segment</span>
                    </>
                  )}
                </button>
              </div>
